...
**版本**：1.2（按 2026-08-30 评审反推修订，对齐实现现状）
**日期**：2026-08-30
**依据文档**：ADR-0001（迁移决策）、ADR-0002（功能范围）、ADR-0003（技术架构）、ADR-0004（网络输出目标，SMB 部分已被 ADR-0007 取代）、ADR-0005/0006/0007（Android 分支与 SMB）、**ADR-0008（v1.0 范围冻结与文档反推原则）**、docs/glossary.md（领域词汇表）

> **文档治理原则（ADR-0008）**：本文档为**桌面版与 Android 版的共同基线**，Android 差异见 `SRS-Android.md`。
> 当规格与实现冲突时默认判定规格过期并修订本文档；判别标准为"是否有用户可感知价值"。
> NFR 一律并列标注 **目标值 / 实测值 / 处置**。实测基线日期 2026-08-30。
...
MediaOrganizer 扫描用户指定的源目录，从照片/视频文件中提取拍摄日期，按"年/月/日"分级目录结构将文件复制或移动到输出目录（本地或网络位置），并提供失败文件管理与可视化正则规则编辑（魔术工具）能力。

**双端发布目标（ADR-0008 决策 1）**：桌面版（Avalonia / Windows）与 Android 版同为 v1.0 发布目标。
桌面为共同基线，Android 差异见 `SRS-Android.md`（本项目文档结构为"主文档 + 平台增补"，SRS-Android 不复制本文件全文）。

**不包含**：照片浏览/相册管理、图片编辑、云端同步、重复文件去重（MD5 指纹）、文件元数据写入、多语言（ADR-0008 决策 2）。
...
开源桌面工具（MIT/Apache 许可证待定）。**v1.0 单一中文界面，不提供多语言**（ADR-0008 决策 2，原"文本资源外置、预留英文"作废）。
UI 文本须集中在 View/XAML 或 VM 的可枚举位置，不得散落进业务逻辑层（NFR-8'），以便将来若重启国际化时可低成本外置。

### 2.2 运行环境

| 项 | 要求 |
|----|------|
| 操作系统 | Windows 10 1809+（桌面主目标）；macOS 12+、主流 Linux 桌面（顺带支持）；**Android 13（API 33）+（见 SRS-Android.md）** |
| 运行时 | 桌面 .NET 10（框架依赖分发，用户需安装运行时）；Android 自包含 APK |
...
| FR-1.5 | 默认跳过 `Hidden` / `System` / `ReparsePoint`（符号链接、挂载点）属性的文件，避免重复计数与链接环；该行为须可配置关闭（v1.0 暂用固定默认值，配置项预留） |
...
| FR-2.4 | 日期统一校验：1970-01-01 ~ 2100-12-31；不早于（当前年份 − max_years_past，默认 30）；不晚于（**当前日期 23:59:59** + future_date_buffer_days，默认 0）。<br>**边界按"日"而非"时刻"判定**：当天拍摄的照片必须判为有效（若按当前时刻判定，早上分析当天下午拍的照片会被误判为未来日期）。校验失败视为该提取器无结果，继续下一个 |
...
> **形态冻结（ADR-0008 决策 2）**：桌面版外壳形态固定为"左侧导航（整理工作台 / 失败文件 / 分析报告 / 设置 / 日志）+ 单页内容区 + 底部状态栏"，**不再补菜单栏**。原 FR-9.1 菜单栏需求与 FR-9.4 语言切换需求已作废。

| 编号 | 需求 |
|------|------|
| FR-9.1 | 导航：左侧导航栏切换页面（整理工作台 / 失败文件 / 分析报告 / 设置 / 日志），功能入口不得遗漏（对应原菜单栏的：打开/保存/加载配置、退出、配置编辑器、魔术工具、主题切换、使用说明、关于、分析报告） |
| FR-9.2 | 日志视图：内存环形日志（上限 1000 条），可刷新/清空 |
| FR-9.3 | 状态栏：聚合当前页面操作反馈（进度、提示、计数），不显示版本/框架等固定信息 |
| ~~FR-9.4~~ | ~~语言切换~~ —— 已删除（ADR-0008 决策 2，v1.0 单一中文） |
...
| FR-10.3 | SMB 走 **SMBLibrary 纯托管客户端**（SMB2，445 端口，NTLM 显式认证，首版不支持 guest/匿名）；WebDAV 走协议客户端，全平台可用。两者均为**双端统一实现**（ADR-0007，取代 ADR-0004 的 UNC 方式） |
| FR-10.4 | 连接配置提供"测试连接"即时验证（认证 + 写权限探测）；**须带超时与取消**，不得阻塞 UI 直到 OS 级 TCP 超时 |
| FR-10.5 | 网络目标下仅提供 copy，move 选项禁用并给出原因提示 |
| FR-10.6 | 流式拷贝，**单次写入载荷不得超过连接协商的写入上限**（SMB = `min(服务器 MaxWriteSize, 1MB)`，该上限由 `SMB2Client.MaxWriteSize` 取得；SMBLibrary 的 `WriteFile` **不**自动分片，超限请求会被服务器直接拒绝）。读取缓冲 1MB，逐文件 + 总体双进度；网络中断自动重试 3 次（指数退避 1/2/4s） |
| FR-10.7 | 传输采用临时名（`<name>.mo-tmp`）+ 完成后改名，杜绝半成品；完成校验 = 大小一致（目标取不到长度时须记为失败而非跳过校验）。**传输或改名失败时须删除 `.mo-tmp` 残留**，目标端不得留下垃圾文件 |
| FR-10.8 | mtime 矫正兼容网络目标：SMB 用 SetFileInformation（FileBasicInformation）写入 LastWriteTime；**WebDAV v1.0 不实现 PROPPATCH，静默跳过**（原"用 PROPPATCH，不支持则跳过"降级为本条，ADR-0008 决策 2） |
| FR-10.9 | 明确不支持 FTP（ADR-0004）。SMB 自 ADR-0007 起双端统一走 SMBLibrary，**不再要求 macOS/Linux 用户自行挂载**（原条款作废） |
...
Avalonia 12 Fluent 风格，不沿用 Qt 布局。**外壳形态已冻结为现状**（ADR-0008 决策 2），原型 `docs/界面原型设计/index.html` 仅作视觉风格参考，不再作为布局基准。要点：

- 主窗口：左侧导航栏（整理工作台 / 失败文件 / 分析报告 / 设置 / 日志）+ 单页内容区 + 底部状态栏；**不设菜单栏**（原"三区块 + 六菜单"描述作废）
  ...
  "networkProfiles": [
    {
      "name": "家里的 NAS",
      "type": "Smb",                  // Smb | WebDav
      "address": "\\\\192.168.1.10\\photos",   // Smb: \\服务器\共享名[\路径]（由 SmbFileStorage 解析，非 OS UNC 重定向器）；WebDav: https://dav.example.com/photos
      "username": "dioha",
...
> **实测基线（2026-08-30）**：NFR 一律并列标注 目标值 / 实测值 / 处置（ADR-0008 决策 3）。

| NFR-6 | 可测试 | Core 关键逻辑（提取、校验、模式引擎、归档计划、重名消解）xunit 覆盖，行覆盖 ≥ 70%。<br>**实测 62.25%（分支 51.19%）——未达标，列为技术债**（ADR-0008 决策 5）。不因未达标下调指标；随高严重度缺陷修复同步补测 |
| NFR-7 | 开源合规 | 第三方依赖许可证清单随 Release 发布（Magick.NET Apache-2.0 等） |
| NFR-8 | 国际化 | **已删除**（ADR-0008 决策 2）。替代约束 **NFR-8'**：UI 文本集中在 View/XAML 或 VM 的可枚举位置，不得散落进业务逻辑层，以便将来低成本外置 |
| NFR-9 | 分发 | GitHub Actions 产出 win-x64 / osx / linux 框架依赖包，附校验和；Android 产出 APK + 校验和 |
| NFR-10 | 安全 | 密码不得明文落盘（Windows DPAPI / Android Keystore）；任何平台日志/报告不得出现凭据；连接测试失败不泄露密码细节。**凭据加密失败须显式报错，不得静默返回空串** |
| NFR-11 | 网络可靠性 | 网络传输单文件失败重试 ≤ 3 次且退避间隔 ≤ 8s；断线不导致 UI 卡死；WebDAV 请求超时默认 30s 可配。<br>**实测：超时可配未实现**（`new Client(...)` 未传超时，退化为 HttpClient 默认 100s）——本阶段补齐，属潜在错误修复 |
...
5. Core 测试与 CI 全部通过；Release 页面可下载桌面三平台包与 Android APK
6. 新增 SMB/WebDAV 连接配置 → 测试连接通过（**不阻塞 UI**）→ 归档到网络目标，结果与本地一致；断网重试后错误清单正确；**失败后目标端无 `.mo-tmp` 残留**
7. 网络目标下 move 选项禁用；密码在 config.json 中为密文（Windows DPAPI / Android Keystore）；**凭据加密失败须显式报错**
8. 当天拍摄的照片不得被判为未来日期；内置的小红书 / OPPO 模式对其目标文件名可正确提取日期
...
| ~~R-6~~ | ~~SMB UNC 仅 Windows 可用~~ | **已解决**：ADR-0007 改用 SMBLibrary 纯托管实现，双端统一、跨平台可用，无需外部挂载 |
| R-7 | macOS/Linux 凭据首版降级存储 | UI 警告 + README 注明；下一版接入系统密钥环 |
| R-8 | SMBLibrary 不做写入分片，且连接/会话有生命周期，易被误用 | 已在 `SmbFileStorage` 内按 `min(协商 MaxWriteSize, 1MB)` 分块（FR-10.6）；连接为 `IDisposable`，所有创建点须 `using`（已发现 2 处泄漏，修复中）。第三方库行为假设须有单元/集成测试兜底 |
| R-9 | 配置文件非原子写可能导致凭据丢失 | `JsonFileStore` 改为"写 .tmp → 原子替换"，损坏时须显式提示而非静默回退出厂设置（修复中） |
